...
import { UnderlinedTitle, InfoTitle } from "./order-info";
...
class OrderSummary extends Component {
    render() {
        ...
        this.props.cartProducts.map(cartProduct => {
            ...
            amStickers += cartProduct.quantity;
        })

        return (
            <div className={`${className} order-summary`}>
                <UnderlinedTitle className="order-summary__title" title="Order Summary" />
                <InfoTitle className="order-summary__subtotal" title={`${amStickers} stickers`} value={`$${subtotal}`} />
